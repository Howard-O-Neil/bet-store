...
export const InfoWallet = () =>async (dispatch: React.Dispatch<ActionType<WalletInfoType>>) => {
    dispatch(GetInfoWallet())
    Axios.defaults.headers.common['Authentication'] = 'Bearer ' + sessionStorage.getItem("token")// for all requests
    let response = await Axios.get<ResponseAPI<WalletInfoType>>(
        `/go/wallet/`
    );
...
const GetInfoWallet_SUCCESS = (wallet: WalletInfoType): ActionType<WalletInfoType> => {
    return {
